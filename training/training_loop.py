...
from dataset.dataset_base import ImageFusionDataset, ImageTestDataset
...
def training_loop(
    ...
    datasets["train"] = ImageFusionDataset(
        opt=config['datasets']['train'])
    # datasets["val"] = VideoFusionDataset(
    #     opt=config['datasets']['test'])
    datasets["val"] = ImageTestDataset(
        opt=config['datasets']['test'])
