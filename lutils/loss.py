...
class L_Gradient(nn.Module):
    ...
    def forward(self, img_f, img_ir):
        ...
        loss = F.l1_loss(grad_f_x, grad_ir_x) + F.l1_loss(grad_f_y, grad_ir_y)
        return loss
...
class Gradient(nn.Module):
    ...
    def forward(self, img):
        ...
        for c in range(img.shape[1]):
            img_pad = F.pad(img[:,c:c+1], (1, 1, 1, 1), mode='replicate')
            grad_x.append(F.conv2d(img_pad, self.sobel_x))
            grad_y.append(F.conv2d(img_pad, self.sobel_y))
        grad_x = torch.cat(grad_x, 1)
        grad_y = torch.cat(grad_y, 1)
        return torch.abs(grad_x), torch.abs(grad_y)
       
class Grad_loss_recons(nn.Module):
    def __init__(self):
        super(Grad_loss_recons, self).__init__()
...
class Grad_loss_fusion(nn.Module):
    def __init__(self):
        super(Grad_loss_fusion, self).__init__()
        self.sobel_x = nn.Parameter(torch.FloatTensor([[-1, 0, 1],
                                                       [-2, 0, 2],
                                                       [-1, 0, 1]]).view(1, 1, 3, 3), requires_grad=False).cuda()
        self.sobel_y = nn.Parameter(torch.FloatTensor([[-1, -2, -1],
...
class WaveletTransform(nn.Module):
    def __init__(self, scale=1, dec=True, params_path='/home/whu/HDD_16T/timer/gmq/MultiModal/WaveDM-main/models/wavelet_weights_c2.pkl', transpose=True):
